...
SECRET_WORD_STR = os.getenv("SECRET_WORD", "")
...
def decrypt_and_parse_vless(encrypted_b64):
    ...
    try:
        ...
        decrypted_str = unpad(decrypted, 16).decode('utf-8')
        return decrypted_str
    except Exception as e:
        print(f"[decrypt] Ошибка расшифрования строки: {e}")
        return None

def decrypt_any_subscription_input(raw_input):
    """
    Дешифрует как одиночную строку, так и полный Base64-бандл (ответ по ссылке /sub/)
    Возвращает список чистых (расшифрованных) ссылок
    """
    raw_input = raw_input.strip()
    if not raw_input:
        return []
    
    # Попробуем расшифровать как одиночную зашифрованную строку
    decrypted = decrypt_and_parse_vless(raw_input)
    if decrypted and decrypted.startswith(("vless://", "vmess://", "trojan://", "ss://")):
        return [decrypted]
        
    # Если не вышло, пробуем как полный бандл (двойной Base64)
    try:
        ...
        print(f"[decrypt_any] Не удалось распознать как бандл: {e}")
